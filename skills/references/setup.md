...
Link this agent to the user's lobster.cash wallet. Use this when the user wants to connect the agent **without** funding or making a purchase — i.e. there is no concrete intent yet to load USDC or create a card.

If the user wants to make a purchase or fund crypto, use `cards request` or `crypto request` instead — they bundle linking automatically. `setup` is the dedicated path for the "just link, nothing else" case.
...
Do not proceed until the user confirms they have approved. Do not poll automatically.
...
If the user originally asked for something specific (e.g. "buy X", "send tokens"), route to the matching section in `SKILL.md`.

## Anti-Patterns

- **Running setup when the user wants to buy or fund:** Use `cards request` or `crypto request` instead — they handle linking automatically.
