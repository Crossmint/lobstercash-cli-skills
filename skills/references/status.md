...
Check agent setup state, wallet balances, and card permissions in one call. Auto-triggers wallet setup if needed.
...
- `cards` — card permissions with phase (`active`, `requires-payment-method`, etc.)
- `ready` — `true` if the agent can pay (has USDC > 0 OR an active card permission)
- `dashboardUrl` — link for the user to manage payments

## Gotchas

- Card permissions do not require USDC — they are backed by the user's credit card. Don't tell the user to "add funds" when using cards.
